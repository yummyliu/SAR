...
如下图（from Hbase：The Definitive Guide），在LSM-tree结构的存储引擎中，每一层都
...
> 这里有个有趣的结论——The Five Minute Rule，即，若某页的访问频率>5min/次，那么可考虑将该页放到内存中。这要求我们在设计存储引擎时，要充分利用内存和外存的存储特点。
