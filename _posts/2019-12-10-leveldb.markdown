...
>
> ![image-20200305134448348](/image/1212-level.png)
>
> 在原论文中，有个有趣的结论——The Five Minute Rule，即，若某页的访问频率>5min/次，那么可考虑将该页放到内存中。这要求我们在设计存储引擎时，要充分利用内存和外存的存储特点。

在LSM-tree结构的存储引擎中，每一层可看做是一个有序的树结构，在进行**rolling merge**的过程中，先从C1取一个multi-page block放到C1-buffer中，其中包含若干个C1的leafnode；然后从C0的leafnode中取entry进行合并，将合并结果追加到C1后面，如下图；新合并的节点不会覆盖原节点，旧节点可用来恢复数据。

![Alt](/image/1212-rollmerge.png)
