...
* TOC
{:toc}
> 本文类似于一个杂乱的笔记，记录认识myrocks过程中的问题
...
基于B+-tree的弊端以及目前存储介质的演化（HDD->SSD，iops更快，但是如果每次写的放大严重，那么会浪费ssd的写性能？），数据库存储引擎也出现了变革。其中LSM-tree目前获得了很多目光，MySQL中也可以集成LSM-tree架构的存储引擎——RocksDB，叫MyRocks。本文是作者对MyRocks学习的总结，也会不断在此更新新的认识，希望对想了解这个产品的同学有所帮助。
...
RocksDb是一个多版本的KV存储引擎（versioned key-value store）；相比于传统数据库以块为最小单位进行IO存储，RocksDB可更细粒度的存储，可认为最小的粒度是kv pair（`(userkey,seq,type) => uservalue`）；
...
除了基本KV操作外，RocksDB提供了了一些额外操作，关键的有：Merge和Compact。
...
Compact是lsm-tree的一个关键操作，之前说过了。rocksdb默认的策略是Leveled Compaction，用在sst file compact中；但是不同层触发条件不同：

+ level0：level0_file_num_compaction_trigger，也是唯一一层允许overlap。
+ Total file size for other

> Universal compaction(sized-tiered)可认为用在flush memtable中；多个overlap的memtable合并为一个ssttable。？

其实我们可以发现，由于L0的key是重叠的，那么L0到L1的Compact不能并发进行；这里通过sub-compaction进行优化，即，先收集文件信息，将单个文件按offset进行分割，最后每个range交由一个subCompact进行处理。

> 见`ShouldFormSubcompactions`决定是否将当前的compact划分成 Sub-Compaction。

rocksdb相比Btree类型，正式因为不需要in-place-update才能有较好的写入速度；但是也正是因为不in-place-update，就存在空间膨胀的问题；就需要进行空间收缩；类似于Vacuum，Compaction主要作用也是回收旧版本并清理已删除的数据；注意，由于Merge 记录的存在，会影响原来[Compact对旧数据的回收](https://github.com/facebook/rocksdb/wiki/Merge-Operator-Implementation#compaction)。关于Compact更详细的逻辑，决定等后面专门整理一篇。

在rocksdb的Compact中，值得一提的是——在过滤数据的时候，可以定义Compaction filter，按照用户定义的逻辑，进行数据整理，比如删除过期（过期时间自定义）数据，转换数据内容等等；Compact filter只对普通value类型（未标记上delete mark）调用，对于merge类型，会在执行merge operator前调用Compaction filter。
...
1. WAL按理说是顺序写的，每次只需要刷最后一个log就行了，为啥要foreach呢？
...
以上，此文持续整理本人在折腾MyRocks过程中，对MyRocks的一些认识和理解，介于理解的深度是由浅入深的，可能目前的版本认识浅薄，甚至有错误，欢迎指正学习。