...
RocksDb是一个多版本的KV存储引擎库（versioned key-value store）；提供了基本的KV操作，每种操作的接口带有相应的Option，比如这些基本操作：
...
KV主要结构是：(userkey,sequence number,type) => uservalue；多版本通过sequence number实现，根据不同KV操作类型产生不同type的数据，主要有kTypeValue、KTypeDelete、KTypeMerge。
...
简单画了下rocksdb的大致架构图，如下图，其中省去了Index、BloomFilter和Block Cache，以及Column Family（下称CF）的概念。

> RocksDB通过CF将(k,v)进行了逻辑上的分区，这样不同分区可以分别写自己的MemTable和SSTTable，但是共享同一个WAL；但是RocksDB保证**跨CF的的原子写**和**一致性读**快照，以及可以分别对不同CF进行挂载、删除与配置。
