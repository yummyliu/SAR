...
上一节提供了DBMS过程模型的简化描述。 我们假定高性能操作系统线程的可用性，并且数据库管理系统将只针对单处理器系统。 在本节的其余部分中，我们放宽第一个假设并描述对DBMS实施的影响。 下一节将讨论多处理和并行处理。

### DBMS Thread

​	今天的大多数数据库管理系统都来自20世纪70年代的研究系统和20世纪80年代的商业化工作。 我们认为理所当然的标准操作系统特性在DBMS开发人员建立原始数据库系统时通常无法使用。 高效率，高规模的操作系统线程支持可能是其中最重要的。 直到20世纪90年代，操作系统线程才得到广泛实施，并且在实际存在的情况下，实现方式差异很大。 即使在今天，一些OS线程的实现也不能很好地扩展到足以支持所有DBMS工作负载[31,48,93,94]。

​	因此，出于传统，可移植性和可伸缩性的原因，许多广泛使用的DBMS在其实现中不依赖于OS线程。 一些完全避免线程，并使用每个DBMS工作者或进程池模型的进程。 那些实现剩余进程模型选择的线程（每个DBMS工作线程模型的线程）需要针对那些没有好的内核线程实现的操作系统的解决方案。 解决这个问题的几个主要的DBMS采用的一种方法是实现他们自己的专有的轻量级线程包。 这些轻量级线程或DBMS线程取代了上一节中描述的操作系统线程的角色。 每个DBMS线程被编程为管理其自己的状态，通过非阻塞异步接口执行所有可能的阻塞操作（例如I / O），并频繁地将控制权交给在这些任务之间分派的调度例程。

​	轻量级线程是一个古老的想法，在[49]中作了回顾性的讨论，并广泛用于用户界面的事件循环编程。 这个概念在最近的OS文献中经常被重新讨论[31,48,93,94]。 这种架构提供了快速任务切换和移植的简便性，代价是在DBMS中复制了大量的操作系统逻辑（任务切换，线程状态管理，调度等）[86]。

### Standard Practice

在今天的领先数据库管理系统中，我们发现了第2.1节介绍的所有三种体系结构的代表以及其中一些有趣的变化。在这个层面上，IBM DB2可能是最有兴趣的例子，因为它支持四种不同的流程模型。在具有良好线程支持的操作系统上，DB2默认为每个DBMS工作者提供线程，并可选择支持在线程池中多路复用的DBMS工作者。当在没有可伸缩线程支持的操作系统上运行时，DB2默认为每个DBMS工作者处理，并且可选地支持在进程池上多路复用的DBMS工作者。
总结IBM DB2，MySQL，Oracle，PostgreSQL和Microsoft SQL Server支持的流程模型：
每个DBMS工作者的进程：
这是最直接的过程模型，今天仍然被大量使用。 DB2默认为在不支持高质量，可伸缩操作系统线程的操作系统上运行每个DBMS工作程序，并在每个DBMS工作线程上运行线程。这也是默认的Oracle流程模型。 Oracle还支持如下所述的流程池作为可选模型。 PostgreSQL仅在所有支持的操作系统上为每个DBMS工作模型运行进程。

每个DBMS worker的线程数：这是一个有两个主要模型的高效模型
今天使用的变体：
1.每个DBMS worker的操作系统线程：在具有良好OS线程支持的系统上运行时，IBM DB2默认使用此模型，这是MySQL使用的模型。
2.每个DBMS工作人员的DBMS线程：在此模型中，DBMS工作人员由OS进程或OS线程上的轻量级线程调度程序调度。该模型避免了任何潜在的操作系统调度程序扩展或性能问题，代价是实施成本高，开发工具支持较差以及DBMS供应商长期存在的软件维护成本。这个模型有两个主要的子类别：
一个。在OS进程上调度的DBMS线程：轻量级线程调度程序由一个或多个OS进程托管。 Sybase和Informix一样支持这种模式。所有使用此模型的当代系统都实现了一个DBMS线程调度程序，该调度程序通过多个OS进程调度DBMS工作程序以利用多个处理器。但是，并非所有使用此模型的DBMS都实现了线程迁移：能够将现有DBMS线程重新分配给不同的OS进程（例如，用于负载平衡）。
湾在OS线程上调度的DBMS线程：Microsoft SQL Server支持将此模型作为非默认选项（默认情况是DBMS工作线程在下面描述的线程池上进行多路复用）。这种称为Fibers的SQL Server选项用于某些高级事务处理基准测试，但否则使用相当少。

进程/线程池：
在此模型中，DBMS工作人员通过一系列进程进行复用。随着操作系统线程支持的改进，该模型的第二种变体已经基于线程池而不是进程池出现。在后一种模式中，DBMS工作人员通过OS线程池进行多路复用：

1. DBMS工作者在进程池上进行多路复用：该模型比每个DBMS工作者的进程具有更高的内存效率，可以在没有良好OS线程支持的情况下轻松移植到OS，并且可以很好地扩展到大量用户。这是Oracle支持的可选模型，以及它们为具有大量同时连接的用户的系统所推荐的模型。 Oracle默认模型是每个DBMS worker的进程。 Oracle支持的两种选项很容易支持它们定位的大量不同操作系统（Oracle支持的目标操作系统超过80种）。
2. DBMS工作人员通过线程池进行多路复用：Microsoft SQL Server默认使用此模型，超过99％的SQL Server安装以此方式运行。为了有效地支持数以万计的并发连接用户，如上所述，SQL Server可以选择支持在OS线程上调度的DBMS线程。
正如我们在下一节中讨论的，大多数当代商用DBMS支持查询内并行性：能够并行地在多个处理器上执行全部或部分单个查询。为了我们在本节中讨论的目的，查询内并行性是将多个DBMS工作者临时分配给单个SQL查询。除了单个客户端连接可能有多个单个DBMS工作程序代表它执行外，其他任何方式都不会影响底层过程模型。

### admission control

我们关闭此部分，其余一个问题与支持多个并发请求有关。 随着任何多用户系统中的工作负载增加，吞吐量将增加到某个最大值。 除此之外，随着系统开始瘫痪，它将开始大幅下降。 与OS一样，颠簸往往是内存压力的结果

DBMS不能将缓冲池中的数据库页面的“工作集”保留下来，并且花费所有时间来替换页面。在DBMS中，查询处理技术尤其是排序和散列连接等查询处理技术会消耗大量主存。在某些情况下，由于争用锁也可能发生DBMS抖动：事务之间不断发生死锁，需要回滚并重新启动[2]。因此，任何好的多用户系统都有一个准入控制策略，除非有足够的DBMS资源可用，否则它不接受新的工作。通过良好的准入控制器，系统将在过载情况下显示优雅的退化：交易延迟将随着到达率成比例地增加，但吞吐量将保持在峰值。
DBMS的准入控制可以在两层完成。首先，简单的准入控制策略可能在调度程序过程中，以确保客户端连接的数量保持在阈值以下。这用于防止像网络连接这样的基本资源过度消耗。在某些DBMS中，假设它是由另一层多层系统处理的，例如应用程序服务器，事务处理监视器或Web服务器，则不提供此控制。
第二层准入控制必须直接在核心DBMS关系查询处理器内实现。这个执行许可控制器在查询被分析和优化之后运行，并确定查询是否被推迟，以更少的资源开始执行，还是开始执行而没有额外的约束。执行准入控制器由查询优化器提供的信息来辅助，该信息用于估计查询将需要的资源以及系统资源的当前可用性。特别是，优化器的查询计划可以指定（1）查询将访问的磁盘设备，以及每个设备的随机和顺序I / O数量的估计，（2）查询的CPU负载估计基于查询计划中的操作符和要处理的元组数量，最重要的是（3）有关查询数据结构的内存占用情况的估计，包括用于在连接和其他查询执行任务期间对大量输入进行排序和散列的空间。如上所述，这个最后的度量标准通常是准入控制器的关键，因为内存压力通常是抖动的主要原因。因此，许多DBMS使用内存占用量和活动DBMS的数量
工人作为准入控制的主要标准。

### discussion Additional Material

过程模型选择对DBMS缩放和可移植性具有重大影响。因此，三个更广泛使用的商业系统各自支持其产品线上的多个流程模型。从工程角度来看，在所有操作系统和所有级别上采用单一过程模型显然会更简单。但是，由于使用模式的多样性和目标操作系统的不一致性，这三个DBMS中的每一个都选择支持多个模型。
展望未来，由于硬件瓶颈的变化以及因特网井的工作负载的规模和可变性，近年来，服务器系统的新流程模型引起了人们的极大兴趣[31,48,93,94]。这些设计中出现的一个主题是将服务器系统分解为一组独立调度的“引擎”，并在这些引擎之间异步传递消息。这就像上面的“进程池”模型，因为工作单元在多个请求中被重用。最近这项研究的主要创新之处在于以比以前更狭窄的任务特定方式打破了功能性的工作粒度。这导致了工作者和SQL请求之间的多对多关系 - 通过多个工作者中的活动来处理单个查询，并且每个工作者针对许多SQL请求执行其自己的专用任务。这种架构可以实现更灵活的调度选择 - 例如，它允许在允许单个工作人员完成许多查询的任务（可能提高总体系统吞吐量）或允许查询在多个工作人员之间取得进步之间进行动态权衡（以提高该查询的延迟）。在某些情况下，这已被证明在处理器高速缓存局部性方面具有优势，并且有能力在硬件中的高速缓存未命中期间保持CPU忙碌。在DBMS上下文中进一步调查这个想法的代表是StagedDB研究项目[35]，这是一个很好的起点，可以进一步阅读。