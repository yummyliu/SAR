---
layout: post
title: raft算法总结与MIT6824的Raft实现
date: 2018-09-24 18:06
...
在集群配置变更的过程中，无法保证所有节点同时变更，因此会出现不同节点对majority的定义不同，如下图：

![image-20191030095402098](/image/1029-raft-membershipchange.png)

因此，在变动过程中，由于同一时刻存在两种配置，会对majority的认识不一致；那么为了保证配置变更时的集群安全，Raft采用两阶段的方式，如下图。

![image-20191030114244543](/image/1029-raft-conf-2pc.png)

相比于其他两阶段的方式，Raft的两阶段不会阻塞客户请求，其添加了一个过渡期（称为joint consensus），在过渡期中，C_old和C_new同时参与决策，有以下三个特点：
...
1. 当leader收到了配置变更的请求后，将C_old和C_new结合，生成一个**特殊的logentry——C_old_new**，将其复制到其他server中；

2. 当follower收到C_old_new记录后，就可以使用C_new的配置了（注意这个C_old_new在follower的生效不会等待commit，而是收到后立马生效；同样地，该记录本身的提交，也是要按照其自己定义的配置达成共识）；但是，此时共识组中可以投票的还是C_old中的server（因为，leader是按照C_old进行决策的）。

   > 此时如果leader挂掉了，由于C_old_new记录并没有commit，那么新的leader可能在C_old中，也可能在C_old_new中；总之，不可能是在C_new中。

3. 当leader按照过渡期的判断（上一段的一致性达成），将C_old_new标记提交后，就进入了joint consensus状态。

4. 之后，leader产生一个C_new的记录，在joint consensus请求提交。

5. 最后，C_new提交，那么集群就完成了配置变更

注意，最后C_new提交的时候，可能leader不在C_new配置中；leader在C_new提交的时候判断，自己是否在C_new中，如果不在，自己降级为follower。

对于被删除的节点A，由于自己不在集群中了，超时没有收到heartbeat，会自己发起RequestVote请求，这样会将当前的leader变成follower，然后在C_new中，选出一个新leader，但是新leader还是不会给A发送hearbeat，那么这个A还会继续骚扰新leader，周次往复；为了避免这个问题，如果节点的election timeout尚未超时，那么本节点认为leader存在，不会理会这个RequestVote请求。
