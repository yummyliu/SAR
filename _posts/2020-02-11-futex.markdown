---
layout: post
title: Overview of concurrency in C++11
...
* TOC
{:toc}
并发编程中最常见的概念就是mutex，这里先简单讲一下mutex；然后，对C++11提供的并发编程框架进行简单介绍，希望对第一次了解C++11并发编程的同学能有简单的帮助。

# 关于mutex

关于多线程并行编程，除了系统得支持线程外，线程之间的同步是最重要的话题，那么线程的同步通常就是基于锁来实现的。这可看做是多线程编程的一个基础设施，那么就从这开始了解。
...
PostgreSQL在9.6版本中，将bufferpool中的spinlock替换为atomic operations（[commitlog](https://commitfest.postgresql.org/9/408/#)），例如从commitlog中找到的下面的变更：
...
其将`refcount`设计成一个原子变量，对其的操作可通过add_fetch原子操作进行，这样减少了自旋锁的代价，比如：
...
在gcc中，提供了类似的操作，比如`__sync_lock_test_and_set`。而在在C++11中有原子变量类型，提供了各种原子操作；原子操作是操作系统提供最小的且不可并行化的指令。

## futex调用

**futex**是**Fast Userspace muTEX**的简称。是在2003由IBM的工程师引入到Kernel中的，主要用于减小高并发下锁同步是system call的代价。
...
CPU在超标量流水线执行中，一个时钟周期会执行多条指令。对于一些硬件上没有相关性的指令，这会造成代码执行顺序与原来不一样；另外在代码编译中，编译器会执行重新排列指令，进行乱序执行的优化；这都会造成实际执行顺序与原来编写的顺序不一致的问题，如果我们要求某些语句的执行顺序需要和看到的一样，这就叫顺序一致的内存模型（Sequential consistent）。

> 不同的处理器架构的内存模型不一样。如果指令严格按照顺序执行，这叫强顺序的架构（x86），否则叫弱顺序（PowerPC，ArmV7等）。那么对于弱顺序的机器，为了保障执行的顺序，在汇编指令中有内存栅栏(memory barrier）的指令，比如PowerPC的sync，在执行到内存栅栏时，会等待之前的指令都执行完成后再执行后面的指令。

编译器的乱序执行的优化对于原子类型默认始终保持与原来一样，这可能会限制并发性能的提升。而如果我们可以对于某些场景并不需要原子类型保证顺序一致性，这该怎么做呢？

在C++11中，通过memory_order来让程序员为原子操作指定内存顺序，有6种：
