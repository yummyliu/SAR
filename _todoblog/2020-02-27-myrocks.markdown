...
MyRocks最初是由FB开发的，目前存在三个主要分支：MariaDB、Percona、FB，[略有不同](https://mariadb.com/kb/en/differences-between-myrocks-variants/)，但大致相似。这里以Percona版本进行介绍，主要feature是：写优化、高压缩的事务型存储引擎（事务隔离级别：RC, RR）。

## Layout

MyRocks的一二级索引是按照KV的方式存储在RocksDB中，其中索引的KV内容如下图：
...
![image-20210202171113003](/image/myrocks-overview/key-format.png)

每个Key由Internal Index ID作为prefix，索引号内部单调递增，维护在系统表`ROCKSDB_GLOBAL_INFO`中；
...
最后，在创建表和索引时，可在INDEX COMMENT中指定CF，CF与index是1：N的关系，如果没有指定CF，将放在default-CF中，可以分别配置每个CF的属性，在系统表information_schema.ROCKSDB_CF_OPTIONS中查看。

### Meta

上面提到默认数据放在default CF中，而MyRocks的元信息单独放在system CF中，每一类元信息由DATA_DICT_TYPE作为前缀存储在rocksdb中，可认为是system cf中的system cf的INDEX ID。
...
- DDL_ENTRY_INDEX_START_NUMBER: create table是创建一条，对应的ddl_manager更新，在重启的时候ddlmanger也会读取该信息更新，（table-defination -> key_definations）
...
- BINLOG_INFO_INDEX_NUMBER，每次commit的时候更新binlog的信息，只有一条记录≈
...
- MAX_INDEX_ID：只有一条记录，记录当前的index_id
...
> Online DDL只支持创建删除二级索引。Instant DDL只支持一些简单的元数据变更，比如默认值，Char(N)等

Create/Drop Table 会更新system中的dict index；

Create/Drop Index会发起专门的Job来创建，并且Inplace Alter（HA_ALTER_INPLACE_SHARED_LOCK_AFTER_PREPARE）。

> h->check_if_supported_inplace_alter() // = **HA_ALTER_INPLACE_SHARED_LOCK_AFTER_PREPARE**
> h->prepare_inplace_alter_table()
> ...
> h->commit_inplace_alter_table

- Create Index: 创建元数据，读取primary key，按照sk进行外部排序（超过RDB_MERGE_BUF_SIZE放在临时文件中）；最后利用SstFileWriter直接写sst文件，并[Ingest](https://github.com/facebook/rocksdb/wiki/Creating-and-Ingesting-SST-files)进LSM-tree。最后提交。
- Drop Index：先把元数据删了，然后注册一个全局drop index ongoing的任务；后面又单独的Rdb_drop_index_thread来处理，处理流程是
