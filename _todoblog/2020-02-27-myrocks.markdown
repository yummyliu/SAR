...
> Online DDL只支持创建删除二级索引。

Create/Drop Table 会更新system中的dict index；

Create/Drop Index会发起专门的Job来创建;

- Create Index: 读取primary key，按照sk进行外部排序（超过RDB_MERGE_BUF_SIZE放在临时文件中）；最后利用SstFileWriter直接写sst文件。
...
- Sequential Scan：利用RocksDB的Iterator
  ...
  3. extra：上层给下层的hint，比如是否只需要读key （HA_EXTRA_KEYREAD）
