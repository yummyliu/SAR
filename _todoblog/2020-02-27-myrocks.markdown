...
如果MyRocks的表开启了TTL特性，并且没有显式指定TTL列，那么在索引记录中会有隐式的TTL列的信息；在PrimaryKey中，位于value的前8个byte。
...
MyRocks的锁粒度是row lock，其锁信息都在内存中；

早期不支持共享锁，对于`select * from t where id = 3 for shared`也是降级为排他锁。
...
并且在MyRocks中，不支持自动死锁检测；

**MyRocks中隔离级别不同不会影响加锁和解锁的行为**
