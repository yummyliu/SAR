...
+ election timeout: 距离上次leader发来信息超过该时间，超时后follower变成candidate，每个follower的超时时间的随机的[150ms, 300ms]
+ election term: 一个选举周期，candidater向其他节点发起投票请求；收到投票请求的节点，如果自己没有发起投票，那么就回复该投票请求，即为该节点投一票，并重置自己的election timeout。
+ Heartbeat timeout：leader会定期发送append entries请求，可视为心跳，给follower。
+ 
...
1. client发送请求给leader
2. leader将变更记在本地log中
3. 下一次心跳中，leader将变更发送给follower
4. leader收到大多数follower的ack，那么该变更标记为commited
5. leader给client返回ack
